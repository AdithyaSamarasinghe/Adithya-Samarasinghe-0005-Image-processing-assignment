# -*- coding: utf-8 -*-
"""Image Processing Assignment .ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1p50Chkg6TCCckRg0V21CmUshI_HUjPsm
...
fig.suptitle('Intensity Transformation for Different Values of a')
...
for i, a in enumerate(a_values):
    ...
    axs[i // 5, i % 5].imshow(image_transformed, vmin=0, vmax=255)
    axs[i // 5, i % 5].set_title(f'a = {a:.2f}')
...
Gx = cv.filter2D(einstein, -1, Px)
Gy = cv.filter2D(einstein, -1, Py)
...
from google.colab import drive
drive.mount('/content/drive')
image1= ['/content/drive/MyDrive/assignment_01_images/a1q5images/im01small.png',
                '/content/drive/MyDrive/assignment_01_images/a1q5images/im02small.png']

image2 = ['/content/drive/MyDrive/assignment_01_images/a1q5images/im01.png',
                '/content/drive/MyDrive/assignment_01_images/a1q5images/im02.png']


no=['1','2']
s=4
for im_path,im2_path,number in zip(image1,image2,no):
    ...
    nearest_neighbourhood=np.sum((zoomed.astype(np.float32) - im2.astype(np.float32)) ** 2) / np.prod(zoomed.shape)
    Bilenear_Interpolation= np.sum((interpolated_image.astype(np.float32) -im2.astype(np.float32)) ** 2) / np.prod(interpolated_image.shape)

    print("Nearest_neighbourhood SSD for image "+number,  nearest_neighbourhood)
    print("Bilenear Interpolation SSD for image "+number,Bilenear_Interpolation)
